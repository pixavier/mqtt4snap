...
The **request** and **response** blocks are suitable for an asynchronous client-server architectural approach on a PubSub based infrastructure.  We add a call identifier (idCall-*) as the callbacktopic at the end of the topic to enable the responder to publish the response. An example of topic is "topicName/readx/idCall-9876.1". The **idCall-xxxx** corresponds to the responseTopic field introduced in [MQTT 5](https://www.emqx.com/en/blog/mqtt5-request-response). We use the 3.1.1 MQTT version to keep compatibility with microcontroller implementations such as the [PubSubClient library](https://www.arduino.cc/reference/en/libraries/pubsubclient).
