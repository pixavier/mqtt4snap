...
The **request** and **response** blocks are suitable for an asynchronous client-server architectural approach on a PubSub based infrastructure. The **idCall** corresponds to the correlation data concept introduced in MQTT5. CallbackTopics are based on **idCall**s.
